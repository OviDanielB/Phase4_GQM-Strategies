...
import it.uniroma2.isssr.repositories.phase42.VariableActivitiRepository;
	...
	public ResponseEntity<?> completeUserTask(String taskIdRun,
		...
		for(MeasureTask measureTask : measureTaskList){
			...
			if(activitiTask.getTaskDefinitionKey().equals(measureTask.getTaskId())){
			  ...
			  TaskVariable state = new TaskVariable();
			  TaskVariable responsible= new TaskVariable();
			  TaskVariable assignee = new TaskVariable();
			  state.setName("state");
			  ...
			  taskVariables.add(0,state);
			  taskVariables.add(1,responsible);
			  taskVariables.add(2,assignee);
				
			  activitiInterationImplementation.createTaskVariableFromRuntimeTaskId(
				  		taskIdRun, taskVariables);
